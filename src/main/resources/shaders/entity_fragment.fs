...
vec4 calcLightColour(vec3 light_colour, float light_intensity, vec3 position, vec3 to_light_dir, vec3 normal) {
    ...
    diffuseColour = diffiseC * vec4(light_colour,1.0) * light_intensity * diffuseFactor;
...
vec4 calcDirectinalLight(DirectionalLight light, vec3 position, vec3 normal) {
...
void main() {
    ...
    vec4 diffuseSpecularComp = calcDirectinalLight(directionalLight, fragPos, fragNormal);
